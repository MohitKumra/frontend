...
export const focusApi = {
  ...
  complete: (id: string) => apiClient.post<FocusSessionDTO>(`/focus/${id}/complete`).then((r) => r.data),
  cancel: (id: string) => apiClient.post<FocusSessionDTO>(`/focus/${id}/cancel`).then((r) => r.data),
  getActive: () => apiClient.get<FocusSessionDTO | null>('/focus/active').then((r) => r.data),
  logTime: (durationMin: number) => apiClient.post('/focus/time-log', { durationMin }).then((r) => r.data),
  listTimeLogs: () =>
    apiClient
      .get<{ id: string; userId: string; durationMin: number; date: string }[]>('/focus/time-logs')
      .then((r) => r.data),
};
