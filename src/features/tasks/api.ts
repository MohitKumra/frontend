...
export const tasksApi = {
  ...
  create: (data: CreateTaskRequest) => apiClient.post<TaskDTO>('/tasks', data).then((r) => r.data),

  update: (id: string, data: UpdateTaskRequest) => apiClient.patch<TaskDTO>(`/tasks/${id}`, data).then((r) => r.data),

  delete: (id: string) => apiClient.delete(`/tasks/${id}`),

  // Subtask endpoints
  listSubTasks: (taskId: string) => apiClient.get<SubTaskDTO[]>(`/tasks/${taskId}/subtasks`).then((r) => r.data),
  ...
  deleteSubTask: (taskId: string, subTaskId: string) => apiClient.delete(`/tasks/${taskId}/subtasks/${subTaskId}`),
