...
import type { ListResponse, NotificationLogDTO, PushSubscriptionRequest, ActivityFeedResponse } from '../../types';

export const notificationsApi = {
  getVapidKey: () => apiClient.get<{ publicKey: string | null }>('/notifications/vapid-key').then((r) => r.data),
  ...
  unsubscribe: () => apiClient.post<{ success: boolean }>('/notifications/unsubscribe').then((r) => r.data),

  getLogs: () => apiClient.get<ListResponse<NotificationLogDTO>>('/notifications/logs').then((r) => r.data),

  markAsRead: () => apiClient.post<{ success: boolean }>('/notifications/read').then((r) => r.data),
