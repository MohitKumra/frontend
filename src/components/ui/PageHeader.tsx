    ...
    <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8 ${className}`}>
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="min-w-0">
          ...
          {subtitle !== undefined && (
            <p className="text-sm sm:text-[0.95rem] text-text-secondary mt-1 font-medium truncate">
              {subtitle}
            </p>
          )}
        </div>
      ...
      {action && (
        <div className="flex items-center shrink-0 self-start sm:self-center sm:mt-0.5">
          {action}
        </div>
      )}
    </div>
