...
import type { CreateTaskRequest, Priority, CreateSubTaskRequest, TaskRecurrenceConfig, TaskStatus } from '../../types';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-black uppercase tracking-[0.18em]" style={{ color: 'var(--color-text-muted)' }}>
          {title}
        </span>
      </div>
      {children}
      ...
      style={
        active
          ? {
              ...
              color: 'white',
              boxShadow: `0 3px 10px ${color ?? 'var(--color-accent)'}44`,
            }
          : {
              background: 'var(--color-surface-raised)',
...
export function CreateTaskModal({
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    // Block submit if recurrence config is invalid
    ...
    try {
      ...
      if (recurrenceConfig) body.recurrenceConfig = recurrenceConfig;
  ...
  const handleTaskParsed = (task: {
    ...
    if (task.recurrenceConfig) setRecurrenceConfig(task.recurrenceConfig);
      ...
      {isOpen && (
        <>
          ...
          >
            {/* Top bar */}
            <div
              className="flex items-center justify-between gap-4 border-b px-4 sm:px-6 py-4 shrink-0"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                    Fill in the details or use AI to build instantly
                  </p>
                </div>
              </div>
              <button
              ...
              <div className="mx-auto w-full max-w-[720px] flex flex-col gap-4 px-4 sm:px-6 py-6">
                ...
                <Section icon={<Clock size={14} />} title="Estimated Duration">
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    ...
                    <Chip active={false} onClick={() => setEstimatedDuration(null)}>
                      Custom
                    </Chip>
                  </div>
                      ...
                      {subTasks.map((s, i) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="rounded-xl p-1.5 transition-opacity hover:opacity-60"
            ...
            {/* Sticky footer */}
            <div
              className="flex items-center justify-between gap-3 border-t px-4 sm:px-6 py-4 shrink-0"
              style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}
            >
              {/* Live summary chips */}
              <div className="flex flex-wrap items-center gap-2 min-w-0 flex-1">
                {title.trim() && (
                  <span
                    className="inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-bold max-w-[180px] truncate"
                    style={{
                      borderColor: 'var(--color-border)',
                ...
                {priority !== 'MEDIUM' && (
                  <span
                    className="hidden sm:inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-bold shrink-0"
                    style={{
                      borderColor: `${PRIORITY_META[priority].color}33`,
                      color: PRIORITY_META[priority].color,
                      background: `${PRIORITY_META[priority].color}12`,
                    }}
                  >
                ...
                {advancedCount > 0 && (
                  <span
                    className="inline-flex items-center gap-1 rounded-full border px-3 py-1 text-xs font-bold shrink-0"
                    style={{
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  disabled={!title.trim() || createTask.isPending || !!validateRecurrenceConfig(recurrenceConfig)}
                  ...
                  style={{
                    ...
                    boxShadow: title.trim()
                      ? '0 4px 16px color-mix(in srgb, var(--color-accent) 30%, transparent)'
                      : 'none',
                  }}
                >
