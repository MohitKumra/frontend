...
export function MobileIslandNav({ onOpenMore }: MobileIslandNavProps) {
  ...
  const collapsedWidth = isCoachPage ? 88 : 124;
  const collapsedHeight = isCoachPage ? 14 : 16;
  ...
  return createPortal(
    <div className="md:hidden pointer-events-none">
      <AnimatePresence>
        {isCoachPage && !isExpanded ? (
          /* On coach page when collapsed: hide island completely, only show subtle bottom arrow-up button */
          <motion.button
            key="coach-bottom-arrow"
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            transition={{ duration: 0.15 }}
            onClick={() => setIsExpanded(true)}
            className="fixed bottom-0 left-1/2 -translate-x-1/2 z-[85] h-5 px-3.5 rounded-t-xl flex items-center justify-center border-t border-x select-none pointer-events-auto shadow-sm active:scale-95 transition-all"
            style={{
          ...
          >
            <ChevronUp size={13} className="hover:text-[var(--color-accent)] transition-colors" />
          </motion.button>
        ) : (
          <div
            ref={containerRef}
          ...
          >
            <motion.div
              animate={{
                ...
                y: isCoachPage && !isExpanded ? 60 : 0,
                opacity: isCoachPage && !isExpanded ? 0 : 1,
              }}
            ...
            >
              {/* ─── SLEEK COLLAPSED LINE / CAPSULE (for non-coach pages) ─── */}
