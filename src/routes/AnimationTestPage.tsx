  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12">
      ...
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-extrabold text-text-primary">Premium Motion Design Test</h1>
        <p className="text-text-secondary">Visual verification of all animation implementations</p>
      </div>
      ...
      <section className="space-y-4">
        ...
        <div
          className="p-6 rounded-2xl border grid grid-cols-1 sm:grid-cols-3 gap-4"
          style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
        >
          <button
            className="px-6 py-3 rounded-xl font-semibold text-white hover:shadow-lg transition-all duration-150"
            style={{ background: 'var(--gradient-accent)' }}
          >
            Primary Button
          </button>
          <button
            className="px-6 py-3 rounded-xl font-semibold border hover:shadow-lg transition-all duration-150"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            ...
            >
              ...
              <p className="text-sm text-text-secondary">Hover to see premium lift animation with shadow</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        >
          ...
          {dropdownOpen && (
            <div
              className="dropdown-enter absolute top-full mt-2 left-6 w-64 p-3 rounded-xl border shadow-lg z-50"
              style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
            >
      ...
      <section className="space-y-4">
        ...
        <div
          className="p-6 rounded-2xl border stagger"
          style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
        >
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="list-item-stagger flex items-center gap-3 p-4 rounded-xl mb-2 border"
      ...
      {modalOpen && (
        ...
        >
          <div
            className="modal-content-enter max-w-md w-full p-6 rounded-2xl shadow-2xl"
          ...
          >
            ...
            <p className="text-text-secondary mb-6">
              This modal uses a premium entrance animation inspired by macOS. Scale from 0.94 to 1.0 with translateY and
              blur.
            </p>
            <button
              onClick={() => setModalOpen(false)}
      ...
      {toastVisible && (
        <div
          className="toast-slide-in fixed top-6 right-6 z-50 max-w-sm p-4 rounded-xl shadow-lg border flex items-start gap-3"
          style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
        >
          <div
