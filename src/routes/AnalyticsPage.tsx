...
function SectionCard({
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 border-b px-5 py-4" style={{ borderColor: 'var(--color-border-subtle)', background: 'var(--color-surface-elevated)' }}>
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl" style={{ background: iconVars[0], color: iconVars[1] }}>
          {icon}
        </div>
        <div className="min-w-0">
          <div className="text-[9.5px] font-bold uppercase tracking-[0.22em]" style={{ color: 'var(--color-text-muted)' }}>
            {eyebrow}
          </div>
          <h3 className="mt-0.5 text-[13.5px] font-bold leading-tight" style={{ color: 'var(--color-text-primary)' }}>
            {title}
          </h3>
          {subtitle && <p className="mt-0.5 text-[10.5px] font-medium leading-snug" style={{ color: 'var(--color-text-muted)' }}>{subtitle}</p>}
        </div>
      </div>
...
function InlineStat({
  ...
  return (
    <div className="rounded-xl border p-3" style={{ background: styles.bg, borderColor: 'var(--color-border-subtle)' }}>
      ...
      {description && <div className="mt-0.5 text-[10px] font-medium leading-snug" style={{ color: 'var(--color-text-muted)' }}>{description}</div>}
    </div>
  );
}
  ...
  return (
    <motion.div whileHover={{ y: -3 }} transition={{ type: 'spring', stiffness: 320, damping: 22 }} className="h-full">
      <Card
        ...
        style={{ borderColor: 'var(--color-border)', boxShadow: '0 1px 2px rgba(15, 23, 42, 0.04), 0 8px 24px -12px rgba(15, 23, 42, 0.08)' }}
      >
        <div className="absolute inset-x-0 top-0 h-[2px]" style={{ background: `var(--gradient-${tone})` }} />
        <div className="relative z-10 flex h-full flex-col">
          <div className="flex items-start justify-between gap-2">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg" style={{ background: iconVars[0], color: iconVars[1] }}>
              {icon}
            </div>
            {typeof trend === 'number' && <TrendPill value={trend} />}
          </div>
          <div className="mt-4 text-2xl font-bold leading-none tabular-nums" style={{ color: 'var(--color-text-primary)' }}>
            {value}
          </div>
          <div className="mt-2 text-[9px] font-bold uppercase tracking-[0.18em]" style={{ color: 'var(--color-text-muted)' }}>
            {label}
          </div>
          {sub && <div className="mt-auto pt-1.5 text-[10.5px] font-medium leading-relaxed" style={{ color: 'var(--color-text-tertiary)' }}>{sub}</div>}
        </div>
      </Card>
    </motion.div>
  );
...
function InsightCard({ insight, index }: { insight: InsightDTO; index: number }) {
  ...
  return (
    <div
      ...
      style={{ background: 'color-mix(in srgb, var(--color-surface) 70%, transparent)', borderColor: 'var(--color-border-subtle)' }}
    >
      <div className="absolute inset-x-0 top-0 h-1" style={{ background: `linear-gradient(90deg, ${tone.fg}, color-mix(in srgb, ${tone.fg} 30%, transparent))` }} />
      <div className="flex items-start gap-3">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-[11px] font-bold" style={{ background: tone.bg, color: tone.fg }}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2.5 text-[12.5px] font-medium leading-relaxed" style={{ color: 'var(--color-text-primary)' }}>
            {insight.text}
          </p>
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.14em]" style={{ background: 'var(--color-border-subtle)', color: 'var(--color-text-muted)' }}>
...
function EmptyInsightsState({
  ...
  return (
    <div className="rounded-2xl border border-dashed p-5" style={{ background: 'color-mix(in srgb, var(--color-surface) 70%, transparent)', borderColor: 'var(--color-border-subtle)' }}>
      <div className="flex items-start gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl" style={{ background: 'var(--color-border-subtle)', color: 'var(--color-text-secondary)' }}>
          {icon}
        </div>
        <div className="min-w-0">
          <h4 className="text-sm font-bold" style={{ color: 'var(--color-text-primary)' }}>{title}</h4>
          <p className="mt-1 text-sm font-medium leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>{description}</p>
        </div>
      </div>
    </div>
  );
...
function PeriodSummaryCard({
  ...
  return (
    <SectionCard
      ...
      className="h-full"
    >
      <p className="text-[13px] font-medium leading-relaxed text-balance" style={{ color: 'var(--color-text-primary)' }}>
        Your score is <span className="text-[15px] font-bold" style={{ color: 'var(--color-accent)' }}>{currentScore}</span>, which places this period {comparison}. The strongest pattern is {leadInsightText.toLowerCase()}, so the cleanest improvement path is to keep task flow and focus time steady.
      </p>

      <div className="mt-4 grid grid-cols-3 gap-3">
        <InlineStat label="Tasks" value={`${tasksCompleted}/${tasksTotal}`} tone={taskDelta >= 0 ? 'success' : 'warning'} />
        <InlineStat label="Focus" value={`${formatMinutes(focusMinutes)} / ${focusSessions} sessions`} tone={focusDelta >= 0 ? 'success' : 'warning'} />
        <InlineStat label="Habits" value={`${habitsCompleted}/${habitsTotal}`} tone={habitDelta >= 0 ? 'success' : 'warning'} />
      </div>

      <div className="mt-4 rounded-xl border p-4" style={{ background: 'var(--color-surface-elevated)', borderColor: 'var(--color-border-subtle)' }}>
        ...
        <p className="mt-1.5 text-[12px] font-medium leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
          The fastest lift usually comes from finishing planned tasks earlier and protecting focused sessions from interruptions.
        </p>
      </div>

      <div className="mt-4 grid grid-cols-2 gap-3">
        <InlineStat label="Consider soon" value={`${comparison.replace('vs ', '')}`} tone="warning" description="Selected date range" />
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="mt-0.5 text-[9px] font-bold uppercase tracking-[0.24em]" style={{ color: 'var(--color-text-muted)' }}>
          Score
        </span>
      </div>
    </div>
...
function MiniStat({ label, value, tone = 'default' }: { label: string; value: string; tone?: 'default' | 'success' | 'warning' | 'info' }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[9px] font-bold uppercase tracking-[0.2em]" style={{ color: 'var(--color-text-muted)' }}>Current score</div>
          <div className="mt-1.5 text-2xl font-bold tracking-tight" style={{ color: 'var(--color-text-primary)' }}>{currentScore}</div>
          <p className="mt-1 text-[11px] font-medium leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
            {hasBenchmarkData ? `Better than ${percentile}% of days in the last 90 days.` : `Not enough history yet. We have ${historyCount} day${historyCount === 1 ? '' : 's'} of data.`}
          </p>
        </div>
      ...
      <div className="mt-4 rounded-xl border p-4" style={{ background: 'var(--color-surface-elevated)', borderColor: 'var(--color-border-subtle)' }}>
        <div className="relative h-9">
          <div className="absolute inset-x-0 top-3.5 h-1.5 rounded-full" style={{ background: 'linear-gradient(90deg, color-mix(in srgb, var(--color-danger) 35%, transparent), color-mix(in srgb, var(--color-warning) 30%, transparent), color-mix(in srgb, var(--color-success) 30%, transparent))' }} />
          {[
            ...
            >
              <span className="h-2.5 w-2.5 rounded-full border-2 border-white shadow-sm" style={{ background: marker.tone }} />
              <span className="mt-1 text-[8px] font-bold uppercase tracking-[0.16em]" style={{ color: 'var(--color-text-muted)' }}>
                {marker.label}
              </span>
            </div>
          ))}
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <InlineStat label="vs best" value={`${deltaFromBest > 0 ? '+' : ''}${Math.round(deltaFromBest)}`} tone={deltaFromBest >= 0 ? 'success' : 'warning'} />
        <InlineStat label="Top" value={`Top ${Math.max(1, 100 - percentile)}%`} tone="info" description="Within current sample" />
      </div>
...
export function AnalyticsPage({ embedded = false }: AnalyticsPageProps) {
  ...
  const previousParams = useMemo(() => ({ startDate: previousRange.startDate, endDate: previousRange.endDate }), [previousRange.startDate, previousRange.endDate]);
  ...
  const { data: previousSummary } = useQuery({
    ...
    queryFn: () => apiClient.get<AnalyticsSummaryDTO>('/analytics/summary', { params: previousParams }).then((r) => r.data),
  });
  ...
  const { data: previousDaily } = useQuery({
    ...
    queryFn: () => apiClient.get<DailyAnalyticsDTO[]>('/analytics/daily', { params: previousParams }).then((r) => r.data),
  });
  ...
  const { data: consistency } = useQuery({
    ...
    queryFn: () => apiClient.get<ConsistencyData>('/analytics/consistency', { params: queryParams }).then((r) => r.data),
  });
  ...
  const focusDelta = percentChange(summary?.focusMinutesTotal ?? 0, previousSummary?.focusMinutesTotal ?? sumDaily(previousDaily).focusMinutes);
  const taskDelta = percentChange(summary?.tasksCompleted ?? 0, previousSummary?.tasksCompleted ?? sumDaily(previousDaily).tasksCompleted);
  const habitDelta = percentChange(summary?.habitsCompletedToday ?? 0, previousSummary?.habitsCompletedToday ?? sumDaily(previousDaily).habitsCompleted);
  ...
  const projectsByStatus = projects?.reduce<Record<string, number>>((acc, project) => {
    acc[project.status] = (acc[project.status] ?? 0) + 1;
    return acc;
  }, {}) ?? {};
  const pieData = Object.entries(projectsByStatus).map(([name, value]) => ({
    name: name.replace('_', ' ').toLowerCase().replace(/^\w/, (char) => char.toUpperCase()),
    value,
  }));
  ...
  const dominantStatusPercent = totalProjectsCount > 0 && dominantStatus ? Math.round((dominantStatus.value / totalProjectsCount) * 100) : 0;
  ...
  const leadInsight = enhancedData?.insights?.find((insight) => insight.type === 'positive') ?? enhancedData?.insights?.[0];
  ...
  const habitBreakdown = Math.round(Math.min((summary?.habitsCompletedToday ?? 0) / Math.max(summary?.habitsTotal ?? 1, 1), 1) * 20);
  ...
  const heroSubtitle = dateRange.startDate === dateRange.endDate
  ...
  return (
    <RootWrapper {...rootProps}>
      ...
      <motion.div variants={itemVariants} className="flex flex-col gap-4">
        <div className="overflow-hidden rounded-[24px] border" style={{ background: 'color-mix(in srgb, var(--color-surface) 70%, transparent)', borderColor: 'var(--color-border-subtle)' }}>
          <div className="grid gap-5 px-6 py-6 lg:grid-cols-[1.1fr_auto] lg:items-start">
            <div className="flex items-start gap-4">
              <div className="hidden h-12 w-12 shrink-0 items-center justify-center rounded-2xl sm:flex" style={{ background: 'linear-gradient(135deg, var(--color-accent), var(--color-info))', color: '#fff' }}>
                <Brain size={22} strokeWidth={1.75} />
              </div>
              <div className="min-w-0">
                <div className="inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.26em]" style={{ background: 'var(--color-accent-subtle)', color: 'var(--color-accent)' }}>
                  <Sparkles size={11} strokeWidth={2} />
                  Executive analytics
                </div>
                <h1 className="mt-2 text-2xl font-bold tracking-tight sm:text-[28px]" style={{ color: 'var(--color-text-primary)' }}>
                  Analytics Dashboard
                </h1>
                <p className="mt-1.5 max-w-2xl text-[13px] font-medium leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
            ...
            <div className="grid gap-2.5 sm:grid-cols-2">
              <div className="rounded-xl border px-4 py-3" style={{ background: 'var(--color-surface-elevated)', borderColor: 'var(--color-border-subtle)' }}>
                <div className="text-[8.5px] font-bold uppercase tracking-[0.22em]" style={{ color: 'var(--color-text-muted)' }}>
                  Period
                </div>
      ...
      <motion.div variants={itemVariants}>
        ...
        >
          <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.1fr_auto_1fr] lg:items-center">
            <div className="min-w-0">
              <div className="inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.2em]" style={{ background: 'var(--color-border-subtle)', color: 'var(--color-text-secondary)' }}>
              ...
              <p className="mt-3 max-w-xl text-[12.5px] font-medium leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
                This is the selected period's productivity score. It blends tasks, habits, focus time, consistency, and peak performance into one number.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 sm:justify-items-start justify-items-center">
              ...
              <HeroStat label="Habits" value={`${summary?.habitsCompletedToday ?? 0}/${summary?.habitsTotal ?? 0}`} />
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 gap-6 xl:grid-cols-[1.5fr_1fr]">
        <SectionCard
          ...
          iconTone="accent"
        >
          <div className="flex flex-wrap items-center gap-3 text-[9.5px] font-bold" style={{ color: 'var(--color-text-muted)' }}>
            <span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-[var(--color-accent)]" />Created</span>
            <span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-[var(--color-success)]" />Completed</span>
            <span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-[var(--color-warning)]" />Overdue</span>
            <span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-[var(--color-info)]" />Habits</span>
          </div>
            ...
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                  ...
                  <XAxis dataKey="label" tick={{ fill: 'var(--color-text-muted)', fontSize: 9, fontWeight: 600 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: 'var(--color-text-muted)', fontSize: 9, fontWeight: 600 }} axisLine={false} tickLine={false} allowDecimals={false} width={24} />
                  <Tooltip contentStyle={{ background: 'var(--color-surface-raised)', border: '1px solid var(--color-border)', borderRadius: 12, boxShadow: 'var(--shadow-md)' }} cursor={{ stroke: 'var(--color-border-subtle)', strokeWidth: 1 }} />
                  <Area type="monotone" dataKey="tasksCreated" name="Created" stroke="var(--color-accent)" strokeWidth={2.5} fill="url(#dailyCreatedFill)" dot={false} activeDot={{ r: 4 }} />
                  <Area type="monotone" dataKey="tasksCompleted" name="Completed" stroke="var(--color-success)" strokeWidth={2.5} fill="url(#dailyCompletedFill)" dot={false} activeDot={{ r: 4 }} />
                  <Area type="monotone" dataKey="tasksOverdue" name="Overdue" stroke="var(--color-warning)" strokeWidth={2.5} fill="url(#dailyOverdueFill)" dot={false} activeDot={{ r: 4 }} />
                  <Area type="monotone" dataKey="habitsCompleted" name="Habits" stroke="var(--color-info)" strokeWidth={2.5} fill="url(#dailyHabitsFill)" dot={false} activeDot={{ r: 4 }} />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
        ...
        <SectionCard
          ...
          subtitle={projects?.length ? `${projects.length} tracked project${projects.length === 1 ? '' : 's'}` : 'No projects yet'}
          ...
          {projects && projects.length > 0 ? (
            <div className="space-y-3">
              {projects.slice(0, 4).map((project, index) => {
                ...
                return (
                  <div key={project.projectId} className="rounded-xl border p-3.5" style={{ background: 'var(--color-surface-elevated)', borderColor: 'var(--color-border-subtle)' }}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h4 className="truncate text-[13px] font-bold" style={{ color: 'var(--color-text-primary)' }}>{project.projectName}</h4>
                        <div className="mt-1.5 flex items-center gap-1.5">
                          <span className="rounded-full px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider" style={{ background: 'var(--color-border-subtle)', color: 'var(--color-text-muted)' }}>
                            {project.status}
                          </span>
                          <ProjectHealthBadge health={project.health} />
                        </div>
                      </div>
                      <div className="shrink-0 text-lg font-bold" style={{ color: accent }}>{Math.round(project.progress)}%</div>
                    </div>
                    ...
                    <div className="mt-3 grid grid-cols-4 gap-2 text-center">
                      <div>
                        <div className="text-[8.5px] font-bold uppercase tracking-[0.16em]" style={{ color: 'var(--color-text-muted)' }}>Expected</div>
                        <div className="mt-0.5 text-[11px] font-bold" style={{ color: 'var(--color-text-primary)' }}>{Math.round(project.expectedProgress)}%</div>
                      </div>
                      <div>
                        <div className="text-[8.5px] font-bold uppercase tracking-[0.16em]" style={{ color: 'var(--color-text-muted)' }}>Delta</div>
                        <div className="mt-0.5 text-[11px] font-bold" style={{ color: project.progressDelta >= 0 ? 'var(--color-success)' : 'var(--color-warning)' }}>{project.progressDelta > 0 ? '+' : ''}{Math.round(project.progressDelta)}%</div>
                      </div>
                      <div>
                        <div className="text-[8.5px] font-bold uppercase tracking-[0.16em]" style={{ color: 'var(--color-text-muted)' }}>Days left</div>
                        <div className="mt-0.5 text-[11px] font-bold" style={{ color: 'var(--color-text-primary)' }}>{project.daysRemaining === null ? 'n/a' : `${project.daysRemaining}d`}</div>
                      </div>
                      <div>
                        <div className="text-[8.5px] font-bold uppercase tracking-[0.16em]" style={{ color: 'var(--color-text-muted)' }}>Focus</div>
                        <div className="mt-0.5 text-[11px] font-bold" style={{ color: 'var(--color-text-primary)' }}>{formatMinutes(project.focusMinutes)}</div>
                      </div>
                    </div>
                    <div className="mt-2.5 text-[9.5px] font-medium" style={{ color: 'var(--color-text-muted)' }}>
                      Expected finish: {project.expectedFinish ? formatReadableDate(project.expectedFinish.slice(0, 10)) : 'n/a'}
                    </div>
                  </div>
                );
              })}
            ...
            <FloatingProjectsEmpty title="No projects yet" description="Create your first project to see schedule health and progress" showCtaHint={false} />
          )}
        </SectionCard>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        ...
        >
            ...
            {[
              ...
              <div key={item.label} className="space-y-1.5">
                <div className="flex items-center justify-between text-[11px]">
                  <span className="font-semibold" style={{ color: 'var(--color-text-secondary)' }}>{item.label}</span>
                  <span
                    className="rounded-full px-1.5 py-0.5 text-[11px] font-bold"
                    style={{
                      color: item.tone === 'accent' ? 'var(--color-accent)' : item.tone === 'success' ? 'var(--color-success)' : item.tone === 'info' ? 'var(--color-info)' : 'var(--color-warning)',
                      background: item.tone === 'accent' ? 'var(--color-accent-subtle)' : item.tone === 'success' ? 'var(--color-success-subtle)' : item.tone === 'info' ? 'var(--color-info-subtle)' : 'var(--color-warning-subtle)',
                    }}
                  >
                    {item.value > 0 ? '+' : ''}{item.value}
                  </span>
                </div>
                <div className="h-2 overflow-hidden rounded-full border" style={{ background: 'var(--color-border-subtle)', borderColor: 'var(--color-border-subtle)' }}>
                  <div
                    className="h-full rounded-full transition-all duration-700"
                    style={{
                      width: item.value > 0 ? `${Math.max(Math.min((item.value / 30) * 100, 100), 6)}%` : '0%',
                      background: item.tone === 'accent' ? 'var(--color-accent)' : item.tone === 'success' ? 'var(--color-success)' : item.tone === 'info' ? 'var(--color-info)' : 'var(--color-warning)',
                    }}
                  />
                </div>
              </div>
          ...
          <div className="mt-5 rounded-xl p-4 text-center" style={{ background: 'color-mix(in srgb, var(--color-accent) 8%, var(--color-surface-raised))' }}>
            <div className="text-[9px] font-bold uppercase tracking-[0.2em]" style={{ color: 'var(--color-text-muted)' }}>Final Score</div>
            <div className="mt-1 text-2xl font-bold" style={{ color: 'var(--color-accent)' }}>{currentScore}</div>
          </div>
        </SectionCard>
            ...
            {weeklyData.length >= 2 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={weeklyData} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fill: 'var(--color-text-muted)', fontSize: 9, fontWeight: 600 }} axisLine={false} tickLine={false} allowDecimals={false} width={24} />
                  <Tooltip contentStyle={{ background: 'var(--color-surface-raised)', border: '1px solid var(--color-border)', borderRadius: 12, boxShadow: 'var(--shadow-md)' }} />
                  <Line type="monotone" dataKey="Tasks" stroke="var(--color-accent)" strokeWidth={2.5} dot={{ r: 3 }} />
                  <Line type="monotone" dataKey="Focus" stroke="var(--color-info)" strokeWidth={2.5} dot={{ r: 3 }} />
                  <Line type="monotone" dataKey="Habits" stroke="var(--color-success)" strokeWidth={2.5} dot={{ r: 3 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : weeklyData.length === 1 ? (
              <div className="flex h-full flex-col items-center justify-center gap-4 rounded-xl border border-dashed p-5 text-center" style={{ borderColor: 'var(--color-border-subtle)' }}>
                ...
                <p className="max-w-[220px] text-[11px] font-medium leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
            ...
            <div className="flex flex-col gap-4">
              <div className="relative h-[160px] w-full">
                ...
                <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-lg font-bold" style={{ color: 'var(--color-text-primary)' }}>{dominantStatusPercent}%</span>
                  <span className="text-[7.5px] font-bold uppercase tracking-[0.16em]" style={{ color: 'var(--color-text-muted)' }}>
                ...
                {pieData.map((entry, index) => (
                  <div key={entry.name} className="flex items-center justify-between text-[11px]">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-semibold" style={{ color: 'var(--color-text-secondary)' }}>{entry.name}</span>
                    </div>
                    <span className="font-bold" style={{ color: 'var(--color-text-primary)' }}>{entry.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ) : (
            <FloatingProjectsEmpty title="No projects yet" description="Create a project to see its distribution here" showCtaHint={false} />
          )}
        </SectionCard>
      </motion.div>
        </motion.div>
