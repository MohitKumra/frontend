...
const AMBIENT_SOUNDS = ['Forest', 'Rain', 'Ocean', 'Silence'];
...
function AmbientSoundCard({ sound, setSound, playing, onToggle, loading, error: hasError }: {
  ...
  const showLoadingWave = loading && playing;
  const showPlayWave = playing && !loading;
    ...
    <Card variant="default" className="p-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="relative flex-1 min-w-0">
          <button
            onClick={() => setOpen((o) => !o)}
            className="text-xs font-semibold text-text-secondary flex items-center gap-1 hover:text-text-primary transition-colors"
            disabled={hasError}
          >
            {loading && playing && (
              <span className="inline-block w-3 h-3 mr-1 rounded-full border-2 border-transparent border-t-accent animate-spin" />
            )}
      ...
      <div className="mt-3 h-6 flex items-end gap-0.5">
        {Array.from({ length: 24 }).map((_, i) => {
          const isShimmer = showLoadingWave;
          const height = `${20 + Math.abs(Math.sin(i * 1.3)) * 80}%`;
          return (
            <span
              key={i}
